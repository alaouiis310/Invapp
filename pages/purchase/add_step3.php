...
$title = "Facture d'Achat - Étape 3/3";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Insert invoice header with image
        $stmt = $conn->prepare("INSERT INTO BUY_INVOICE_HEADER (
            INVOICE_NUMBER, 
            SUPPLIER_NAME, 
            TOTAL_PRICE_TTC, 
            TOTAL_PRICE_HT, 
        ...
        // Get invoice image from session
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error saving invoice header: " . $stmt->error);
        }
        ...
        foreach ($_SESSION['purchase_products'] as $product) {
            ...
            $detailStmt = $conn->prepare("INSERT INTO BUY_INVOICE_DETAILS (
                ID_INVOICE,
                PRODUCT_ID,
                PRODUCT_NAME,
                QUANTITY,
            ...
            if (!$detailStmt->execute()) {
                throw new Exception("Error saving invoice details: " . $detailStmt->error);
            }
            $detailStmt->close();
        }
        ...
        // Set success message
        $_SESSION['success_message'] = "Facture d'achat enregistrée avec succès!";
...
<h1>Nouvelle Facture d'Achat - Étape 3/3</h1>
...
<form action="" method="post">
    <div class="form-actions">
        <a href="add_step2.php" class="btn btn-secondary">Retour</a>
