...
if (!isset($_SESSION['credit_note'])) {
    redirect('credit_note_step1.php');
}

$title = "Avoir/Retour d'Achat - Étape 2/3";
include '../../includes/header.php';

// Handle adding products
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    if (!isset($_SESSION['credit_note_products'])) {
        $_SESSION['credit_note_products'] = [];
    }
    ...
    $_SESSION['credit_note_products'][] = $product;
}

// Handle product removal
if (isset($_GET['remove']) && isset($_SESSION['credit_note_products'])) {
    $index = (int)$_GET['remove'];
    if (isset($_SESSION['credit_note_products'][$index])) {
        array_splice($_SESSION['credit_note_products'], $index, 1);
    }
    redirect('credit_note_step2.php');
}

// Get products from the original invoice
$originalProducts = [];
$sql = "SELECT PRODUCT_ID, PRODUCT_NAME FROM BUY_INVOICE_DETAILS WHERE ID_INVOICE = 
        (SELECT ID_INVOICE FROM BUY_INVOICE_HEADER WHERE INVOICE_NUMBER = ?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $_SESSION['credit_note']['invoice_number']);
$stmt->execute();
...
<h1>Nouvel Avoir/Retour d'Achat - Étape 2/3</h1>
...
<?php if (!empty($_SESSION['credit_note_products'])): ?>
    ...
    <table>
        ...
        <tbody>
            ...
            foreach ($_SESSION['credit_note_products'] as $index => $product): 
    ...
    <div class="form-actions">
        <a href="credit_note_step1.php" class="btn btn-secondary">Retour</a>
        <a href="credit_note_step3.php" class="btn btn-primary">Suivant</a>
    </div>
<?php else: ?>
