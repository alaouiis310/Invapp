...
-- For SELL_INVOICE_HEADER table
ALTER TABLE SELL_INVOICE_HEADER 
ADD COLUMN invoice_prefix VARCHAR(10) NOT NULL DEFAULT 'INV' AFTER INVOICE_NUMBER,
ADD COLUMN invoice_sequence INT NOT NULL AFTER invoice_prefix;

-- For BON_LIVRAISON_VENTE_HEADER table
ALTER TABLE BON_LIVRAISON_VENTE_HEADER 
ADD COLUMN delivery_prefix VARCHAR(10) NOT NULL DEFAULT 'BLV' AFTER ID_BON,
ADD COLUMN delivery_sequence INT NOT NULL AFTER delivery_prefix;

-- For APP_SETTINGS table (to store starting numbers)
INSERT INTO APP_SETTINGS (SETTING_KEY, SETTING_VALUE) VALUES 
('INVOICE_START_NUMBER', '1'),
('DELIVERY_START_NUMBER', '1')
ON DUPLICATE KEY UPDATE SETTING_VALUE = VALUES(SETTING_VALUE);
...
-- Table for delivery credit note headers
CREATE TABLE IF NOT EXISTS DELIVERY_CREDIT_NOTE_HEADER (
    ID_CREDIT_NOTE INT AUTO_INCREMENT PRIMARY KEY,
    CREDIT_NOTE_NUMBER VARCHAR(50) NOT NULL UNIQUE,
    ...
    IMAGE LONGBLOB,
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (CLIENT_NAME) REFERENCES CLIENT(CLIENTNAME)
);

-- Table for delivery credit note details
CREATE TABLE IF NOT EXISTS DELIVERY_CREDIT_NOTE_DETAILS (
    ID_DETAIL INT AUTO_INCREMENT PRIMARY KEY,
    ID_CREDIT_NOTE INT NOT NULL,
    PRODUCT_ID VARCHAR(50) NOT NULL,
    PRODUCT_NAME VARCHAR(100) NOT NULL,
    ...
    UNIT_PRICE_TTC DECIMAL(10,2) NOT NULL,
    TOTAL_PRICE_TTC DECIMAL(10,2) NOT NULL,
    FOREIGN KEY (ID_CREDIT_NOTE) REFERENCES DELIVERY_CREDIT_NOTE_HEADER(ID_CREDIT_NOTE),
    FOREIGN KEY (PRODUCT_ID) REFERENCES PRODUCT(REFERENCE)
);